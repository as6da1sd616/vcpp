...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		PAINTSTRUCT ps;
		HDC hdc = BeginPaint(hwnd, &ps);  // BeginPaint 함수를 사용하여 hdc를 가져옴

		RECT tempRect; // tempRect를 이곳에 선언

		if (isKeyPressed)
		{
			FillRect(hdc, &rect_target, hBrush_target);  // 먼저 분홍색 네모 그리기
			FillRect(hdc, &rect_user, hBrush_user);      // 그 다음 파란색 네모 그리기

			// 두 상자가 겹쳐 있는 경우에만 텍스트를 출력
			if (IntersectRect(&tempRect, &rect_user, &rect_target))
			{
		...
		else
		{
			...
			FillRect(hdc, &rect_user, hBrush_eraser);    // 먼저 파란색 네모를 지우고, 그 다음 분홍색 네모 그리기
		}

		EndPaint(hwnd, &ps);  // EndPaint 함수를 호출하여 hdc 정리
	}
	break;
	case WM_CLOSE:
		DestroyWindow(hwnd);
